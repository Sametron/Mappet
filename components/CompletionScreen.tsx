...
const CompletionScreen: React.FC<CompletionScreenProps> = ({ show, onAnimationComplete }) => {
  const [step, setStep] = useState(0);

  useEffect(() => {
    if (show) {
      setStep(0); // Reset on show
      const timers: number[] = [];
      // Animation sequence
      ...
      timers.push(window.setTimeout(() => setStep(3), 3200));  // 3. Trigger exit animations
      timers.push(window.setTimeout(onAnimationComplete, 3800)); // 4. Transition to next screen after exit animation starts
  ...
  const isExiting           = step >= 3;

  return (
    <div className={`fixed inset-0 bg-black/80 backdrop-blur-lg flex flex-col items-center justify-center z-50 overflow-hidden ${isExiting ? 'animate-fadeOut' : 'animate-fadeIn'}`}>
      ...
      <div className="relative text-center z-10 w-full h-full flex flex-col items-center justify-center py-12 md:py-16">
        {/* Congrats Header */}
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
          {ISLAND_CONFIGS.map((island, index) => {
            const getCardClass = () => {
              ...
              if (isExiting) classes += ' card-exiting';
              return classes;
            };
            ...
            const style = {
              ...
              transitionDelay: isExiting ? '0ms' : `${index * 150}ms`,
              zIndex: 10 + index,
      ...
      <style>{`
        ...
        .animate-fadeOut { animation: fadeOut 0.6s ease-in forwards; }
        ...
        .card-wrapper {
          ...
          aspect-ratio: 7/12;
          /* Initial state: start from centre of container, off-screen */
          opacity: 0;
        ...
        .card-in-formation {
          opacity: 1;
          /* Final position in the fan formation */
        ...
        .card-exiting {
          /* Exit state: fly up and off-screen */
          opacity: 0;
          transform: translate(-50%, -50%) translateY(-50vh) scale(0.5);
          transition: transform 0.6s cubic-bezier(0.6, -0.28, 0.735, 0.045), opacity 0.5s ease-in;
        }
      `}</style>
    </div>
  );
